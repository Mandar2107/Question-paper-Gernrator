...
function Profile() {
    ...
    const updateProfile = async () => {
        ...
        await updateDoc(userRef, user)
            ...
            .catch(err => console.log(err))
        if (pass) {
            updatePassword(currentUser, pass).then((res) => {
                alert("Password Updated")
            })
                .catch((err) => console.log(err))
        }
        // db.collection('Users').doc(currentUser.uid).update(user).then(res=>alert("Updated succeessfully"))
    }
    ...
    const uploadProfile = async () => {
        ...
        if (!file) alert("Please upload file")
